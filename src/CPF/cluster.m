...
function C = cluster(S,minEuclidian,minTheta,minNeighbours)
Cl = 0;
C = {};
...
for p = 1:size(S,1)
    ...
    ang
    indx = find(not(points)&((d<minEuclidian)&(d>0)&(ang'<minTheta))');
    N = S(indx,:);
    i = 1;
    M = size(N,1);
    if(M<minNeighbours) 
    ...
    while(i<M+1)
        ...
        neigh = points(((d2<minEuclidian)&(ang'<minTheta)))==0;
